...
namespace CsvImport.Product
{
    public class ProductManager : IProductManager
    {
        ...
        private async Task SetOperationTypesAsync(bool replaceAll, List<CsvOperationModel> csvOperations, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var existingProducts = await _productRepository.GetAllBySkusAsync(csvOperations.Select(co => co.CsvItem.Sku), cancellationToken);
            foreach (var csvOperation in csvOperations)
            {
                if (existingProducts.Any(x => x.Sku == csvOperation.CsvItem.Sku))
                {
                    csvOperation.ProductId = existingProducts.Single(x => x.Sku == csvOperation.CsvItem.Sku).Id;
            ...
            if (replaceAll)
            {
                var productsToRemove = await _productRepository.GetAllExceptSkusAsync(csvOperations.Select(co => co.CsvItem.Sku), cancellationToken);
